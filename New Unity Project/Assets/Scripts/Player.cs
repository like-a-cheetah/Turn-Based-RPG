...
public class Player : MovingObject
{
    ...
    public float swordPoint;
    public float magicPoint;
    public float bowPoint;
    ...
    void Update()
    {
            ...
            Input.GetKey(KeyCode.DownArrow) || Input.GetKey(KeyCode.RightArrow) || Input.GetKey(KeyCode.LeftArrow))
        {
            ...
            {
                if (useBow) //활 버튼 누르고, 스페이스 바를 누를 경우 바라보고 있는 방향으로 화살이 날아감
                {
                    if (horizontal == 0)
                        StartCoroutine(ShootArrow(vertical));
                    else if (vertical == 0)
                        StartCoroutine(ShootSideArrow(horizontal));
                    else
                    {
                        Debug.Log("cross");
                        StartCoroutine(ShootCross(horizontal, vertical));
                    }
                }
                else    //활 장착이 안 돼있다면 바라보는 방향으로 공격,
                {
                    AttemptMove<Enemy>(horizontal, vertical);
                    StartCoroutine(EnemyTurn());
                }
            }
            else    //방향키, PageUp, PageDown, Home, End키의 값을 받아옴
            {
                horizontal = (int)Input.GetAxisRaw("Horizontal");
                vertical = (int)Input.GetAxisRaw("Vertical");
